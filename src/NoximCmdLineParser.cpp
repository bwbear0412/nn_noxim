...
void showHelp(char selfname[]) {
    ...
    cout << "\t-groupsize N\tSet the number of neuron in each group to the specified integer value (default " << DEFAULT_GROUP_NEU_NUM << ")" << endl;
    //**************************************//
    //** 2018.09.02 edit by Yueh-Chi,Yang **//
    cout << "\t-NNmodel  FILENAME	Neural Network Model with table in the specified file named 'FILENAME.txt' (default: " << DEFAULT_NNMODEL_FILENAME << ")" << endl;
    cout << "\t-NNweight  FILENAME	Neural Network Weight with table in the specified file named 'FILENAME.txt' (default: " << DEFAULT_NNWEIGHT_FILENAME << ")" << endl;
    cout << "\t-mapping_table  FILENAME	Mapping table in the specified file named 'FILENAME.txt' (No default)" << endl;
    cout << "\t-mapping  algorithm	Mapping algorithm name (default: " << DEFAULT_MAPPING_ALGORITHM << ")" << endl;
    cout << "\t-NNinput  FILENAME	Mapping algorithm with table in the specified file named 'FILENAME.txt' (default: " << DEFAULT_NNINPUT_FILENAME << ")" << endl;
    //**************************************//
    ...
    cout << "\t\ttable FILENAME\tRouting Table Based routing algorithm with table in the specified file" << endl << endl;
    ...
    cout << "\t-pir R TYPE\t\tSet the packet injection rate to the specified real value [0..1] (default " << DEFAULT_PACKET_INJECTION_RATE << ") and the time distribution \n\t\t\t\tof traffic to TYPE where TYPE is one of the following:" << endl;
    ...
    cout << "\t\tcustom R\tCustom distribution with given real probability of retransmission" << endl << endl;
    ...
    cout << "\t\ttable FILENAME\tTraffic Table Based traffic distribution with table in the specified file" << endl << endl;
...
void checkInputParameters() {
    ...
    if(NoximGlobalParams::min_packet_size < 2 || NoximGlobalParams::max_packet_size < 2) {
        cerr << "Error: packet size must be >= 2" << endl;
        exit(1);
    }

    if(NoximGlobalParams::min_packet_size > NoximGlobalParams::max_packet_size) {
    ...
    if(NoximGlobalParams::packet_injection_rate <= 0.0 || NoximGlobalParams::packet_injection_rate > 1.0) {
    ...
    for(unsigned int i = 0; i < NoximGlobalParams::hotspots.size(); i++) {
        if(NoximGlobalParams::hotspots[i].first >= NoximGlobalParams::mesh_dim_x * NoximGlobalParams::mesh_dim_y) {
            cerr << "Error: hotspot node " << NoximGlobalParams::hotspots[i].first << " is invalid (out of range)" << endl;
            exit(1);
        }
        if(NoximGlobalParams::hotspots[i].second < 0.0 && NoximGlobalParams::hotspots[i].second > 1.0) {
    ...
    if(NoximGlobalParams::stats_warm_up_time > NoximGlobalParams::simulation_time) {
...
//****************tyty*********************//
                if(!strcmp(routing, "xyx")) {
                    NoximGlobalParams::routing_algorithm = ROUTING_XYX;
//*****************************************//
                }
                    ...
                    NoximGlobalParams::routing_algorithm = ROUTING_PROPOSED;
                    ...
                    NoximGlobalParams::routing_algorithm = ROUTING_DOWNWARD_CROSS_LAYER;
                }
                else if(!strcmp(routing, "DLADR_HS")) {
                    NoximGlobalParams::routing_algorithm = ROUTING_DOWNWARD_CROSS_LAYER_HS;
                }
                ...
                if(!strcmp(distribution, "poisson")) {
                    ...
                    NoximGlobalParams::probability_of_retransmission = NoximGlobalParams::packet_injection_rate * pow((1 - r), (1 / Aoff - 1 / Aon));
                }
                ...
                if(!strcmp(traffic, "random")) {
                    ...
                    NoximGlobalParams::traffic_distribution = TRAFFIC_RANDOM_TVAR;
                }
                else if(!strcmp(arg_vet[i + 1], "random_2")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_RANDOM_2;
                    /****************MODIFY BY HUI-SHUN********************/
                }
                else if(!strcmp(traffic, "transpose1")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_TRANSPOSE1;
                }
                else if(!strcmp(traffic, "transpose2")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_TRANSPOSE2;
                }
                else if(!strcmp(traffic, "bitreversal")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_BIT_REVERSAL;
                }
                else if(!strcmp(traffic, "butterfly")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_BUTTERFLY;
                }
                else if(!strcmp(traffic, "shuffle")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_SHUFFLE;
                }
                else if(!strcmp(traffic, "table")) {
                    NoximGlobalParams::traffic_distribution = TRAFFIC_TABLE_BASED;
                    strcpy(NoximGlobalParams::traffic_table_filename, arg_vet[++i]);
                }
                else {
                ...
                NoximGlobalParams::max_volume_to_be_drained = atoi(arg_vet[++i]);
            }
