...
// int throttling[DEFAULT_MESH_DIM_X][DEFAULT_MESH_DIM_Y][DEFAULT_MESH_DIM_Z];
...
void NoximNoC::buildMesh() {
    ...
    for(int i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
        for(int j = 0; j < NoximGlobalParams::mesh_dim_y; j++) {
            for(int k = 0; k < NoximGlobalParams::mesh_dim_z; k++) {
                ...
                t[i][j][k]->DW_tag_neighbor[DIRECTION_NORTH](DW_tag_to_south[i][j][NoximGlobalParams::mesh_dim_z - 1]);
    ...
    /*******DOWNWARD ROUTING******/
    for(int k = 0; k <= NoximGlobalParams::mesh_dim_z - 2; k++) {
        for(int j = 0; j <= NoximGlobalParams::mesh_dim_y; j++) {
            for(int i = 0; i <= NoximGlobalParams::mesh_dim_x; i++) {
...
void NoximNoC::entry() {
    ...
    if(reset.read()) {
        ...
        if((getCurrentCycleNum() % (int) (TEMP_REPORT_PERIOD)) == 0) {
            ...
            bool IsEmergency = EmergencyDecision();
    ...
    if(getCurrentCycleNum() == NoximGlobalParams::simulation_time) {
