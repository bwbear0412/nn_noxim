...
int sc_main(int arg_num, char *arg_vet[]) {
            ...
            for(int y = 0; y < NoximGlobalParams::mesh_dim_y; y++) {
                for(int x = 0; x < NoximGlobalParams::mesh_dim_x; x++) {
                    if(!(n->t[x][y][z]->r->buffer[d].IsEmpty())) {
                        ...
                        double    tt   = 0;
                        tt = NoximGlobalParams::simulation_time + NoximGlobalParams::stats_warm_up_time -
                             flit.timestamp;
                        results_buffer << tt << "\t";
                    }
                    else {
    ...
    if((NoximGlobalParams::max_volume_to_be_drained > 0) && (getCurrentCycleNum() / 1000 >= NoximGlobalParams::simulation_time * CYCLE_PERIOD)) {
        cout << "\nWARNING! the number of flits specified with -volume option" << endl;
        cout << "has not been reached. ( " << drained_volume << " instead of " << NoximGlobalParams::max_volume_to_be_drained << " )" << endl;
